...
class ControleRemoto:
    ...
    def tv(self, msg=''):
        if self.On_Off:
            linhas = [f'[black on red] {x} [/]' if x ==
                      self.canal_atual else f'{x}' for x in range(1, len(self.canais)+1)]
    ...
    def comando(self, msg=''):
        msg = input(f'< CH1 >   - VOL{self.volume} +')
        while True:
            if self.On_Off == False:
                ...
                if msg in '-+' and self.volume <= 3:
                    ...
                    if msg == '>':
                        if self.canal_atual == len(self.canais):
                            self.canal_atual = 1
                            return self.tv()
                        else:
                            self.canal_atual += 1
                        ...
                        if self.canal_atual == 1:
                            self.canal_atual = len(self.canais)
                            return self.tv()
                        else:
                            self.canal_atual -= 1
                            return self.tv()
